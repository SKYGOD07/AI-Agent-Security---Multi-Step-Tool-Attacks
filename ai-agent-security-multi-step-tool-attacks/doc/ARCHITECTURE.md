# System Architecture: scenario Discovery Engine

This document details the modular architectural layout and data-flow pipelines of the scenario Discovery Engine.
...
graph TD
    ...
    Ranker -->|sorts & filters| Output[scenario Candidates]
```
...
| Chess Engine Concept (Stockfish) | scenario Discovery Engine Equivalent | Target File |
|---|---|---|
...
| **Move Generator** | Candidate Generator (mutators, test vector banks) | `attack_discovery/mutation.py` |
| **Position Evaluation** | Heuristic score and Novelty bonus calculations | `attack_discovery/search.py` |
| **Best Move** | Ranked scenario Candidate | `attack_discovery/candidate.py` |
...
| **Principal Variation** | Highest-scoring tool-use sequences sequence | `attack_discovery/candidate.py` |
...
5.  **Ranking**: Findings that trigger predicates are compiled, scored by behavior weight, and sorted by `CandidateRanker` to form the final portfolio.
