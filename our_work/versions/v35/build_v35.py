"""Build OMEGA v5 notebook: omega_v5_attack.ipynb

Reads omega_v5_attack.py, base64-encodes it, and builds a 3-cell Kaggle
...
PROJECT_ROOT = DIR.parents[2]  # our_work/versions/v35 -> root

# Read attack source
attack_src = (DIR / "omega_v5_attack.py").read_text(encoding="utf-8")
...
output_path = PROJECT_ROOT / "our_work" / "notebooks" / "omega_v5_attack.ipynb"
with open(output_path, "w", encoding="utf-8") as f:
    json.dump(nb, f, indent=1)

# ===== Validation =====
print("=" * 60)
print("OMEGA v5 (Final Push) Notebook Validation")
print("=" * 60)
...
checks = [
    ...
    ("<|end|><|start|>assistant<|channel|>analysis<|message|><|end|>", "Verbose Harmony frame template"),
]
...
# v5-specific checks
print(f"\n--- OMEGA v5-Specific Validations ---")
v5_checks = [
    ("REPLAY_SAFE_FRAC = 0.99", "Keeps proven 0.99 replay fraction"),
    ("SPLIT_CLASSIFY_N = 3", "v5 uses 3-sample classification (v4 was 5)"),
    ("SLOWEST_MULT = 1.20", "v5 uses 1.20 slowest mult (v4 was 1.28)"),
    ("max_tool_hops=1", "Warm-up at hops=1"),
    ("warmup_fired", "Warm-up candidate harvesting"),
]

for pattern, desc in v5_checks:
    assert pattern in decoded, f"FAIL: {desc}"
    print(f"[OK] {desc}")

# CRITICAL: verify the double-counting bug is FIXED
assert "replay_cost += (time.monotonic() - run_start)" not in decoded, \
    "FAIL: v4 double-counting bug NOT fixed — warm-up time still charged as replay cost"
print("[OK] v4 double-counting bug FIXED (warm-up not charged as replay cost)")

# Safety knobs
assert "REPLAY_SAFE_SIZING = True" in decoded
assert "BURST_K = 1" in decoded
assert "SLOW_MULTIPOST_N = 1" in decoded
assert "FALLBACK_N = 300" in decoded
...
print("=" * 60)
print("ALL VALIDATIONS PASSED -- OMEGA v5 notebook is ready!")
print("=" * 60)
