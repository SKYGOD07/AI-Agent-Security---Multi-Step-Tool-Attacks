"""OMEGA v35 (v65) — TURBO-BACKFILL: v22 champion + fast-row seed reseed (Avenue 3).

ONE change vs the byte-exact v22 (91.305) engine: on the FAST row (gemma) the
stop-rule seed `slowest` is lowered from the fixed 12.0s to the measured median of
the first few REAL single-post fast-row candidate latencies (~8.5s). v22 pins
`slowest`>=12.0 on gemma even though real latency is ~8.5s, so its `slowest*1.10`
cushion idles a few candidates. Reseeding reclaims them WITHIN v22's exact caps
(0.945/0.985 primary, 0.995/0.997 backfill are UNCHANGED) — so the replay-timeout
surface is identical to v22 (NOT the extreme-edge cap over-extension that broke v34).
Reseed only ever LOWERS slowest, only on the fast row (slow row is byte-identical
to v22), and uses only plain-TEMPLATE candidate latencies (never exotic probes,
per blacklist rule #5). Worst case (median at or above 12.0s) it is a no-op == v22.
