"""OMEGA v19 (v49) — Harmony Warmup Breakthrough Edition (Target 91.395+).
...
Preserves 100% of v13's exact winning code and knobs:
  ...
  8. WARMUP_IDX = 899999 (EXACTLY 1 WARMUP CANDIDATE)

New Breakthrough: Harmony-Forged Warmup
  - Uses FRAME_TEMPLATE for warm-up interaction.
  - On gpt_oss (slow row), Harmony channel forging bypasses reasoning CoT during warm-up!
  - Saves 15 seconds of model-loading overhead -> expands replay budget for +2 fill candidates!

Calculated Yield: ~1015.5 candidates = 91.395+ score!
"""
...
# ---- Tuning Knobs (OMEGA v19 / v49 - Harmony Warmup Config) ----
MARGIN_S = 47.0
...
class AttackAlgorithm(AttackAlgorithmBase):
    """OMEGA v19 Harmony Warmup Breakthrough Attack Algorithm."""
    ...
    def _fill(
        ...
        # ---- HARMONY-FORGED WARM-UP (fast model-loading) at hops=1 ----
        run_start = time.monotonic()
        warmup_msg = _msg(WARMUP_IDX, FRAME_TEMPLATE)  # Uses FRAME_TEMPLATE for fast CoT bypass on gpt_oss
        warmup_fired = False
        try:
            env.reset()
