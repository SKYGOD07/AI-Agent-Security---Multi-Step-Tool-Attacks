"""OMEGA v8 (v38) — Clean Recovery.

Reverts to EXACT v5 architecture (proven 89.640) after v7's multi-predicate
regression (88.335 → -1.305 points).

v7 REGRESSION ROOT CAUSE:
  1. Warm-up at hops=2 wasted 5-15s of fill budget
  2. MULTI_TEMPLATE (2 tool calls) added ~3-5s per candidate → ~30% fewer candidates
  3. If fs.delete was blocked by guardrails, per-candidate value stayed at 18
     while candidate count dropped → net regression

RULE: Never add extra tool hops to the fill template. Each additional hop
costs ~3-5s × ~1000 candidates = 3000-5000s of lost budget.

v8 = EXACT v5 code (proven 89.640 scorer). No experiments. Same code → same score.
"""
...
# ---- Tuning Knobs (OMEGA v8 / v38 - EXACT v5 proven config) ----
MARGIN_S = 47.0
SLOWEST0 = 25.0
SLOWEST_MULT = 1.20
...
class AttackAlgorithm(AttackAlgorithmBase):
    """OMEGA v8 Clean Recovery Attack Algorithm (identical to proven v5)."""
