"""Build OMEGA v22 (BACKFILL) notebook: omega_v22_attack.ipynb"""
...
attack_src = (DIR / "omega_v22_attack.py").read_text(encoding="utf-8")
...
output_path = PROJECT_ROOT / "our_work" / "notebooks" / "omega_v22_attack.ipynb"
...
# ===== Validation =====
print("=" * 60)
print("OMEGA v22 BACKFILL Validation")
print("=" * 60)
...
for pattern, desc in [
    ...
    ("PRIMARY_WALL_FRAC = 0.985", "Primary segment wall guard"),
    ("BACKFILL_REPLAY_FRAC = 0.995", "Backfill stays under 100% even pessimistically"),
    ("BACKFILL_WALL_FRAC = 0.997", "Backfill wall extension"),
...
# Two-phase loop structure present
assert "for replay_cap, wall_deadline in caps:" in decoded
assert decoded.count("caps = [") >= 2 or "caps = [" in decoded
print("[OK] Two-segment phase loop present")
...
ab_section = decoded[decoded.index("BOUNDED SLOW-ROW A/B"):decoded.index("TWO-SEGMENT")]
...
print("=" * 60)
print("ALL VALIDATIONS PASSED -- OMEGA v22 BACKFILL notebook is ready!")
print("=" * 60)
