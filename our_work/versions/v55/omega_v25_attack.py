...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def _fill(self, env: Any, budget: float, max_hops: int) -> list[AttackCandidate]:
        ...
        # ADAPTIVE SIZING: p90 of the recent committed-template latency window
        # replaces the pessimistic running-max for stop projections. A single
        # GPU spike no longer permanently inflates the cushion (the documented
        # v20 failure mode), while the hard generation-deadline guard below
        # still uses the global max so the search phase can never overrun.
        ...
        def next_est() -> float:
            if len(recent) >= 8:
                ...
                return max(p90 * mult, LAT_FLOOR_S)
            return slowest * mult
        ...
        for replay_cap, wall_deadline in caps:
            while len(banked) < cap:
                ...
                if replay_cost + next_wall * coef >= replay_cap:
                    break
                now = time.monotonic()
                if now + next_wall >= wall_deadline:
                    break
                # HARD generation-budget guard: never attempt an interaction
                # that could cross the wall even under the global maximum.
                if now + slowest * 1.05 >= wall_deadline:
                    break
