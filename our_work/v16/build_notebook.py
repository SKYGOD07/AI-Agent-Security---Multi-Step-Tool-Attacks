"""v16 Notebook Builder: exact reproduction of the 88.560 aiagsec-ea-b-0721.ipynb structure.

Uses `%%writefile` for attack.py inline instead of base64 encoding.
Uses KAGGLE_IS_COMPETITION_RERUN to gate the server start.
Matches the exact markdown cells.
"""
...
def create_v16_notebook(output_path: Path | None = None) -> Path:
    project_root = Path(r"c:\OPENAI AI AGENT CYBERSEC\our_work")
    if output_path is None:
        output_path = project_root / "stockfish_v16_attack.ipynb"
    ...
    print(f"v16 Notebook built at: {output_path}")
