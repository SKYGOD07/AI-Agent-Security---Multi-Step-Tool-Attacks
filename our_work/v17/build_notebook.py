"""v17 Notebook Builder: 89.055 logic inside v16 structural wrapper.

Uses `%%writefile` for attack.py inline instead of base64 encoding.
Uses KAGGLE_IS_COMPETITION_RERUN to gate the server start.
Matches the exact 5-cell structure that Kaggle accepts without format errors.
"""
...
def create_v17_notebook(output_path: Path | None = None) -> Path:
    project_root = Path(r"c:\OPENAI AI AGENT CYBERSEC\our_work")
    if output_path is None:
        output_path = project_root / "stockfish_v17_attack.ipynb"

    attack_py_path = project_root / "v17" / "attack_v17.py"
    ...
    cell_1 = {
        ...
        "metadata": {"tags": []},
        "source": [
            "# AI Agent Security — Multi-Step Tool Attacks · **Submission notebook v17**\n",
            "\n",
            "This notebook uses the algorithmic strength of the 89.055 template-race notebook\n",
            "but wraps it in the structurally bulletproof %%writefile format to avoid submission errors.\n",
            "\n",
    ...
    cell_2 = {
        ...
        "metadata": {"tags": []},
        "source": [
            "### 1 · Write `attack.py` (Uniform Template Race)\n"
        ]
    }
    ...
    print(f"v17 Notebook built at: {output_path}")
