...
- **Status**: **PREFERRED** (The core of all 85+ scores)
...
- **Confidence**: 80% (Theoretically sound, awaits final v20 validation).
- **Replay Risk**: Low
- **Notebook Risk**: None
...
- **When it failed**: In v15 and v19, static/unverified diversity padding caused budget dilution and replay timeouts.
- **Confidence**: 60%
- **Replay Risk**: Medium (Non-firing templates might cause long tool-chains during replay).
- **Notebook Risk**: None

## 5. Hop-Saturation (BURST_K)
- **Status**: **BLACKLISTED**
- **Purpose**: Force the LLM to execute multiple tool calls (e.g., 4 `http.post` calls) per interaction to artificially inflate cell count.
- **Mechanics**: Appending instructions like "Call http.post exactly 4 times".
- **When it failed**: v18 (Score dropped to 57). Target models struggle with complex multi-step tool logic and drop out of tool-use mode.
- **Confidence**: 0%
- **Replay Risk**: High
- **Notebook Risk**: None
