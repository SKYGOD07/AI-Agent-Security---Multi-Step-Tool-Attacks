...
# Strict regex replacements (ordered longest/most specific to shortest)
REPLACEMENTS = [
    # Explicit title / exact competition references to preserve in part or sanitize prose around
...
# Patterns that preserve code signatures/filenames/Kaggle terms
PRESERVE_SUBSTRINGS = [
    "AttackAlgorithm",
    "AttackCandidate",
    "aicomp_sdk",
...
def clean_text(text):
    # We will split text into chunks or process line by line to protect code signatures
    ...
    for line in lines:
        # Check if line contains preserved code elements that shouldn't be altered
        # e.g., python code lines like `class AttackAlgorithm:`, `from aicomp_sdk import ...`, `submission attack.py`
        has_preserved = any(p in line for p in PRESERVE_SUBSTRINGS)
        
        # Also check for filenames like omega_v13_attack.py or notebooks
        if re.search(r'omega_v\d+_attack\.(py|ipynb)', line) or re.search(r'stockfish_v\d+_attack\.(py|ipynb)', line):
            has_preserved = True
            
        if has_preserved:
            # Only do safe non-code replacements on line if strictly isolated
            # Or preserve line as is
    ...
    return "".join(new_lines), modified_count

print("Clean text engine initialized.")
