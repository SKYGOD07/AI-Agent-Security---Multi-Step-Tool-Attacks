...
def clean_cell_source(source_lines):
    new_lines = []
    cell_mods = 0
        ...
        if file.endswith('.ipynb'):
            ...
            try:
                ...
                for cell in nb.get('cells', []):
                    # Clean markdown cells or code comments
                    ...
                    if cell_type == 'markdown':
                        new_source_lines, mods = clean_cell_source(source_lines)
                        nb_mods += mods
                        cell['source'] = "".join(new_source_lines) if is_str else new_source_lines
                    elif cell_type == 'code':
                        # Clean code comments only
                        new_source_lines = []
                        for line in source_lines:
                            if line.strip().startswith('#'):
                                cleaned_line, mods = clean_cell_source([line])
                                nb_mods += mods
                                new_source_lines.append(cleaned_line[0])
                            else:
                ...
                if nb_mods > 0:
                    ...
                    modified_notebooks.append((rel_path, nb_mods))
            except Exception as e:
                print(f"Error processing notebook {rel_path}: {e}")

print(f"Notebook cleanup complete. Total modified notebooks: {len(modified_notebooks)}")
print(json.dumps(modified_notebooks[:20], indent=2))
